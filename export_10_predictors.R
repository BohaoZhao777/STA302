# Export AmesHousing data with 10 specific predictors
library(AmesHousing)
library(dplyr)
...
# Define the 10 predictor variables as requested
...
# Create the 10 predictor variables list
predictor_vars <- c(
...
cat("\n=== Creating Dataset ===\n")
ames_10_predictors <- ames %>%
  select(all_of(available_vars)) %>%
  na.omit()

cat("Dataset created with", nrow(ames_10_predictors), "rows and", ncol(ames_10_predictors), "columns\n")

# Display data summary
cat("\n=== Data Summary ===\n")
cat("Response variable (Sale_Price):\n")
cat("- Mean: $", round(mean(ames_10_predictors$Sale_Price), 2), "\n")
cat("- Median: $", round(median(ames_10_predictors$Sale_Price), 2), "\n")
cat("- Range: $", min(ames_10_predictors$Sale_Price), " - $", max(ames_10_predictors$Sale_Price), "\n")

# Categorical variables summary
cat("\nCategorical variables:\n")
if("Street" %in% colnames(ames_10_predictors)) {
  cat("- Street levels:", length(unique(ames_10_predictors$Street)), "(", paste(unique(ames_10_predictors$Street), collapse = ", "), ")\n")
}
if("Utilities" %in% colnames(ames_10_predictors)) {
  cat("- Utilities levels:", length(unique(ames_10_predictors$Utilities)), "(", paste(unique(ames_10_predictors$Utilities), collapse = ", "), ")\n")
}
...
for(var in numerical_vars) {
  if(var %in% colnames(ames_10_predictors)) {
    cat("-", var, ": mean =", round(mean(ames_10_predictors[[var]], na.rm = TRUE), 2), 
        ", range =", round(min(ames_10_predictors[[var]], na.rm = TRUE), 2), "-", 
        round(max(ames_10_predictors[[var]], na.rm = TRUE), 2), "\n")
  }
}

# Save the dataset
cat("\n=== Saving Dataset ===\n")
write.csv(ames_10_predictors, "ames_10_predictors.csv", row.names = FALSE)
cat("✓ Dataset saved as: ames_10_predictors.csv\n")
cat("✓ File contains", nrow(ames_10_predictors), "rows and", ncol(ames_10_predictors), "columns\n")

# Display first few rows
cat("\n=== Data Preview (First 3 rows) ===\n")
print(head(ames_10_predictors, 3))

cat("\n=== Export Complete ===\n")
cat("The new CSV file 'ames_10_predictors.csv' has been created with the requested 10 predictor variables!\n")
