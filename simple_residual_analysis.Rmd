...
cat("Note: This file contains 10 predictor variables for model analysis\n")
...
cat("Note: Wood_Deck_SF not available in current dataset\n")
```
...
# Fit model with 10 predictors including interaction term
mod <- lm(
...
cat("Model with 10 predictors and Street:Garage_Cars interaction:\n")
summary(mod)
...
cat("Note: Wood_Deck_SF not included as it's not available in the dataset\n")
```
...
ggsave("residual_plots_10_predictors.png", final_grid, width = 16, height = 12, dpi = 300)
cat("Image saved as: residual_plots_10_predictors.png\n")
cat("This includes plots for all 10 predictors plus fitted values\n")
cat("Note: Wood_Deck_SF not included as it's not available in the dataset\n")
```
